...
0. Make sure you’re in the right project (`import_template_{USERNAME}`) and use the `Git` workflow by `Pull`ing to check for any changes. Then, create a new Quarto document, delete the default text, and save this document.
...
You can also change the base map style. For websites, `addProviderTiles()` is often more reliable than directly calling external WMS services. Here we use a topographic-style provider map and draw circle markers on top.
...
We can also layer a second reference map on top of the base map. In this example, we add labels over satellite imagery using common provider tiles that tend to render more consistently on published Quarto websites.
...
::: callout-tip
### Write up a quarto document to report the plots

1. Save the `qmd` you have been working on for this lesson.
2. Duplicate the qmd and rename it a05.qmd in the assignments directory, which you will edit
3. Your report should contain two images: one plot of the data and one map of the locations. Pick the versions from the lesson that you like best.
4. Each line of code that makes the plot and map needs a comment line explaining what it does
5. You also need an introduction that motivates the analysis and a discussion that comments on the images. 
6. "Render" the `qmd`. You will upload the html, qmd, and some other files that enable the plot and map to your GitHub repo.
7. Use the `Git` workflow: ```Stage > Commit > Pull > Push```
